...
public class AllItemsAdminAdapter extends RecyclerView.Adapter<AllItemsAdminAdapter.ViewHolder> {
    List<Item> itemList;
    private Context context;
    CommonDBCall commonDBCall = new CommonDBCall();
    public AllItemsAdminAdapter(List<Item> itemList1, Context context) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View view= layoutInflater.inflate(R.layout.all_items_admin, parent, false);
//        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.single_item, parent,false);
        return new AllItemsAdminAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AllItemsAdminAdapter.ViewHolder holder, int position) {
...
//        });
        ...
        holder.addItemToLatest.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                commonDBCall.addItemToLatestAdmin(itemList.get(position).getName(), itemList.get(position).getDescripton(), itemList.get(position).getPrice(), itemList.get(position).getImage(), itemList.get(position).getDoc_id(), context);
            }
        });
        holder.addItemToTop.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                commonDBCall.addItemToTopAdmin(itemList.get(position).getName(), itemList.get(position).getDescripton(), itemList.get(position).getPrice(), itemList.get(position).getImage(), itemList.get(position).getDoc_id(), context);
            }
        });
    }
